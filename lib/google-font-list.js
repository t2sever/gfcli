...
/** @type {number} */
const FONT_LIST_MAX_BYTES = 5 * 1024 * 1024;
 ...
 */
function GoogleFontList(options) {
	if (!(this instanceof GoogleFontList))
		return new GoogleFontList(options);
	...
	/** @type {boolean} */
	this.loaded = false;
	if (!options || !options.skipInitialLoad) {
		this.load(false);
	}
}
...
GoogleFontList.prototype.clone = function(){
	var newFontList = new GoogleFontList({ skipInitialLoad: true });
	newFontList.data = this.data;
	newFontList._filterField = this._filterField;
	newFontList._filterTerm = this._filterTerm;
	newFontList.loaded = this.loaded;
	return newFontList;
}
