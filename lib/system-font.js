...
SystemFont.prototype._saveTmp = async function(remoteFile, fileName) {
	...
	const filePath = path.join(folder, safeFileName + remoteExt);
...
/**
 * Convert API-provided font and variant names into a single safe filename.
 * @param {string} fileName
 * @returns {string}
 */
SystemFont.prototype._sanitizeFileName = function(fileName) {
...
SystemFont.prototype._isValidFontFile = function(mimeType, ext) {
	...
	if (mimeType) {
		const ttfMimes = ['application/font-sfnt', 'font/ttf', 'font/sfnt'];
		if (ext === '.ttf' && ttfMimes.includes(mimeType.mime)) return true;
